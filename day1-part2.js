...
function optimizedSum2020 (input) {
    ...
    for (let i = 0; i < arr.length - 2; i++) {
        ...
        while (leftPointer < rightPointer) {
            if ((arr[i] + arr[leftPointer] + arr[rightPointer]) === 2020) {
                return (arr[i] * arr[leftPointer] * arr[rightPointer])
            }
            else if ((arr[i] + arr[leftPointer] + arr[rightPointer]) < 2020) {
                leftPointer++
            }
            else {
                rightPointer--
...
// Naive approach
function naiveSum2020 (input) {
    for (let i = 0; i < input.length - 1; i++) {
        for (let j = 1; j < input.length - 1; j++) {
            for (let k = 2; k < input.length -1; k++) {
...
console.log('this is optimal', optimizedSum2020(data))
console.log('this is naive', naiveSum2020(data))